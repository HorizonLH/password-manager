...
编辑弹窗按「基本信息 / 凭据 / SAP GUI 登录 / 密码规则 / 密码循环与历史 / 备注」**分区折叠**，
默认只展开前面两块 —— 只想改标题或密码时不用再滚过整张表单。弹窗打开时焦点会被限制在弹窗内
（`Tab` 循环），关闭后回到刚才那个按钮。
...
快捷键：`Ctrl+K` 聚焦当前页的筛选框（账号页按标题 / 用户名 / 备注，同步文件页按键，
关联关系页按账号 / 文件 / 键）、`Ctrl+N` 新建条目、`Ctrl+L` 锁定。

「分类」与「SAP Logon 系统」用的是**和「同步文件」页的账号选择器同一套控件**（同样是浮层列表、
上下键 / 回车 / `Esc`，只是列表短，不带搜索框，直接按首字母跳转）。
...
2. **点选密码键并绑定账号**：文件内容以折叠树（JSON / YAML / XML）或扁平表格
   ...
   键多的时候用「文件内容」上方的**筛选框**（`Ctrl+K`）按键路径或值过滤 ——
   过滤时折叠树会自动全部展开，并显示「N / M 个键匹配」。
...
- **控件高度**：`--control-h`（32px）统一输入框、下拉框、按钮与选择器的身高，
  表单同一行的标签与控件始终顶对齐（审计里有几何断言，见 3.2）。
...
`sapshcut` 参数组装、`.sap` 文件内容与前后端字段契约（当前 86 个用例）。
...
另外还有一条**几何断言**：同一个 `.form__grid` 同一行里所有控件（输入框 / 下拉框 / 按钮）
的顶边与高度差必须 ≤ 1px —— 表单对齐一旦退化，审计会直接报 FAIL。
...
1.2.3 又补上：备注往返（编辑 → 保存 → 备注还在）、`Ctrl+K` 在三个页面各自的落点、
弹窗焦点陷阱与关闭后的焦点还原、分类 / SAP Logon 系统两个下拉的选择，以及
导入保险库弹窗在主密码错误时保持打开、可以在同一个弹窗里重试。
