...
**注释永远不会被当成值，也永远不会被改写。** 每种格式都按自己的注释规则解析：

| 格式 | 注释写法 |
| --- | --- |
| TOML / YAML | `#`（引号内的 `#` 不算注释） |
| INI / `.properties` | `#`、`;`、`!`，行首或前面有空格时才算注释 |
| `.env` | 行首 `#` 或前面有空格的 `#`，所以 `PASSWORD=ab#cd` 仍是完整密码 |
| `.tfvars` / `.hcl` | `#`、`//`、`/* … */` |
| JSONC | `//`、`/* … */`（严格 JSON 本来就没有注释） |
| XML | `<!-- … -->` |

YAML 的 `key: |` / `key: >` 文本块整体按内容处理，里面写什么都不会被解析成键。
因此：被注释掉的 `password = "旧密码"` 既不会出现在界面的键列表里，也不在同步范围内；
行尾注释（`password = "old" # 说明`）在同步之后仍留在原处。这些都有单元测试守着。
...
- 只改被绑定的键：值在原地按字节区间替换，其余内容（注释、键顺序、缩进、其它键）一个字节都不动；
...
5. 点「同步」把账号密码写回文件；同步后界面会重新读取文件，键列表里的值与
   「将更新 / 已最新」标记立刻反映磁盘上的真实内容。「关联关系」页可以总览所有绑定。
