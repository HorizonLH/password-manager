...
use crate::model::{LinkOrigin, SyncTarget, Vault};
...
pub fn build_context(vault: &Vault, line_separator: &str) -> AppResult<Value> {
    ...
    for entry in &vault.entries {
        ...
        let username_password = format!("{username}{line_separator}{}", entry.password);
        let value = json!({
            "id": entry.id,
            "title": entry.title,
        ...
        for link in &entry.links {
            files.push(json!({
                ...
                "host": link
                    .evidence
                    .as_ref()
                    .map(|evidence| evidence.host.clone())
                    .unwrap_or_default(),
...
pub fn presets() -> Vec<TemplatePreset> {
    vec![
        TemplatePreset {
            ...
            description: "标准 JSON，含 accounts 与 contentFiles，可直接被 MCP 服务读取"
                .to_string(),
            template: r#"{
  "sapVault": {
...
"#
        ...
        TemplatePreset {
            ...
            label: ".env".to_string(),
            description: "按系统 ID 展开的环境变量，适合脚本与命令行工具".to_string(),
            template: r#"# SapVault 自动生成于 {{generatedAt}}
SAP_KNOX_ID={{knoxId}}
{{#accounts}}SAP_{{systemId}}_USER={{username}}
SAP_{{systemId}}_PASSWORD={{password}}
SAP_{{systemId}}_CLIENT={{client}}
SAP_{{systemId}}_HOST={{hosts}}
{{/accounts}}"#
                .to_string(),
        },
        TemplatePreset {
...
{{/accounts}}"#
        ...
        TemplatePreset {
            ...
            label: "CSV".to_string(),
            description: "系统ID、客户端、用户名、密码、主机，便于导入其它工具".to_string(),
            template: r#"系统ID,客户端,语言,用户名,密码,主机,标题
{{#accounts}}{{systemId}},{{client}},{{language}},{{username}},{{password}},{{#hosts}}{{.}}|{{/hosts}},{{title}}
{{/accounts}}"#
                .to_string(),
        },
        TemplatePreset {
            ...
            label: "纯文本".to_string(),
            description: "人眼可读的账号清单".to_string(),
            template: r#"SapVault 导出（{{generatedAt}}）
Knox ID：{{knoxId}}
账号数：{{accountCount}}    关联文件数：{{fileCount}}
...
mod tests {
    ...
    use crate::model::{Category, ContentLink, Entry, SapAccount};
    ...
    fn sample_vault() -> Vault {
        ...
        first.links.push(ContentLink::from_path(
            "C:/demo/mcp.json",
            LinkOrigin::Scan,
            None,
        ));
    ...
    fn renders_variables_filters_and_sections() {
        ...
        assert_eq!(
            render(
                "{{#accounts}}{{number}}/{{#isLast}}last{{/isLast}}/{{/accounts}}",
                &context
            )
            .unwrap(),
            "1//2/last/"
    ...
    #[test]
    fn username_password_uses_the_configured_separator() {
        let context = build_context(&sample_vault(), "\r\n").unwrap();
        assert_eq!(
            render("{{#accounts}}{{usernamePassword}}{{/accounts}}", &context).unwrap(),
            "JDOE\r\nS3cret!KNOX01\r\nS3cret!"
        );
    }

    #[test]
    fn every_preset_renders_and_mcp_json_stays_valid() {
        ...
        for preset in presets() {
            ...
            if preset.format == "mcpJson" {
                let parsed: Value = serde_json::from_str(&output)
                    .unwrap_or_else(|err| panic!("mcpJson 不是合法 JSON: {err}\n{output}"));
                assert_eq!(parsed["sapVault"]["accounts"].as_array().unwrap().len(), 2);
                assert_eq!(
                    parsed["sapVault"]["contentFiles"].as_array().unwrap().len(),
                    1
                );
            }
        }
